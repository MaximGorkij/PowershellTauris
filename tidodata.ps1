<#
.SYNOPSIS
    Stiahnutie dochadzky z TiDo API za predchadzajuci den do CSV suboru.
.DESCRIPTION
    Skript sa prihlasi do TiDo API pomocou API klienta, ziska autentifikacny hash
    a nasledne stiahne data o odpracovanych hodinach za predchadzajuci den.
    Vystupny CSV subor je ulozeny do /mnt/tido-data/ s datumom v nazve.
.NOTES
    Verzia:    1.1
    Autor:     Marek Findrik
    Pozadovane moduly: -
...
# --- Stiahnuti dat za predchadzajuci den ---
$datum = (Get-Date).AddDays(-1).ToString('yyyy-MM-dd')
